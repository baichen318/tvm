...
def cast_np(np_in, dtype):
    ...
    """
    def cast(val):
        if isinstance(dtype, (type.Fixed, type.UFixed)):
            bits = dtype.bits
            fracs = dtype.fracs
            val = int(val * (1 << fracs))
            mod = val % (1 << bits)
            if isinstance(dtype, type.Fixed):
                val = mod if mod < (1 << (bits-1)) else mod - (1 << bits)
            else:
                val = mod
            val = float(val) / (1 << fracs)
            return val
        elif isinstance(dtype, (type.Int, type.UInt)):
            bits = dtype.bits
            mod = int(val) % (1 << bits)
            if isinstance(dtype, type.Int):
                val = mod if mod < (1 << (bits-1)) else mod - (1 << bits)
            else:
                val = mod
            return val
...
def pack_np(np_in, dtype_in, dtype_out):
    ...
    np_out = []
    signed = True
    if isinstance(dtype_in, (type.UInt, type.UFixed)):
        signed = False
...
def unpack_np(np_in, dtype_in, dtype_out):
    ...
    np_out = []
    signed = True
    if isinstance(dtype_out, (type.UInt, type.UFixed)):
        signed = False
    for i in range(0, shape[0] * factor):
